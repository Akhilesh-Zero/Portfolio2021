...
function resize() {
  ...
  ctx.translate(ca.width * 0.5, ca.height * 0.5);
  ctx.lineWidth = 5;
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
}
...
ca.addEventListener("mousemove", function (e) {
  angleOffsetGoal = Math.PI2 * (e.clientX / ca.width - 0.5);
});
ca.addEventListener("mouseout", function (e) {
  angleOffsetGoal = 0;
});
...
function loop() {
  ...
  ctx.clearRect(-ca.width * 0.5, -ca.height * 0.5, ca.width, ca.height);
  ...
  var rotationZ = Math.PI * 0.5 * Math.cos(t);
  // The "twist" quantity is how much between 0 and 1 the circles should be
  var twist = twistEasing((Math.cos(t * 2 + Math.PI) + 1) * 0.5),
